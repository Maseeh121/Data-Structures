...
public:
  ...
  void insert(TKey key, TValue value)
  {
    ...
    while (!nodes.empty())
    {
      ...
      int HL = -1;   // -1 if empty (nullptr)
      int HR = -1;   // -1 if empty (nullptr)


      if (cur->Left != nullptr){
        HL = cur->Left->Height;  // Get Height of Left sub-tree
      }
      if (cur->Right != nullptr){
        HR = cur->Right->Height; // Get Height of Right sub-tree
      }

      int newH = 1 + max(HL, HR);

      cur->Height = newH;   // Update height of node
