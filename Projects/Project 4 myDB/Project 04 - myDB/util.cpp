...
vector<streamoff> LinearSearch(string tablename, int recordSize, int numColumns, string matchValue, int matchColumn)
{
  ...
  while (pos < length)
  {
    ...
    {
      data >> value;
      if ((i == matchColumn) && (value == matchValue)){
...
// GetAllRecords
//
// This function goes through all the contents of a .data file and
// inserts the value at matchColumn, as well as the streamoff position for that record,
// in the avl tree that was passed by reference
//
void GetAllRecords(avltree<string, streamoff> &tree, int recordSize, int numColumns, int matchColumn, string tablename)
{
  ...
  //
  // Okay, read file record by record, and store value and pos in tree:
  //
  data.seekg(0, data.end);  // move to the end to get length of file:
  streamoff length = data.tellg();
  ...
  while (pos < length)
  {
    ...
    for (int i = 0; i < numColumns; ++i)  // read values, one per column:
    {
      data >> value;
      if (i == matchColumn){
        tree.insert(value, pos);  // insert the value and streamoff pos
      }
    }
